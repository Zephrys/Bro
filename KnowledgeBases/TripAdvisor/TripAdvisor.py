...
def moreReviewHandler(persistent, name, response):
    ...
    ratings = [x['alt']
               for x in soup('img', {'class': 'sprite-rating_s_fill'})]
    ratingDates = [x.text for x in soup('span', {'class': 'ratingDate'})]
    partials = [x.text for x in soup('p', {'class': 'partial_entry'})
                if x.parent['class'][0] == 'entry']

    for x in xrange(0, len(partials)):
        data = {'review': partials[x], 'rating': ratings[x],
                'ratingDates': ratingDates[x]}
...
def reviewHandler(persistent, hotel_url, keyword, rating, response):
    ...
    for url in urls:
        ...
        data = {'askForConfirmation': 'false', 'mode': 'filterReviews',
                'q': keyword, 'returnTo': url}
        persistent['moreReviews'].append({'url': url, 'data': data,
                                          'name': name})
...
def getReviews(keyword, place, entityType):
    ...
    sleep(1) 
    url = url % (quote(keyword,safe=""), tripadvisor_code, entityMap[entityType])
    response = requests.get(url)
    ...
    for url in persistent['urls']:
        ...
        url = 'https://www.tripadvisor.com' + url
        persistent['i'] += 1
        binding = functools.partial(reviewHandler, persistent,
                                    url, keyword, rating)
        data = {'askForConfirmation': 'false', 'mode': 'filterReviews',
                'q': keyword, 'returnTo': url}
    ...
    for urls in persistent['moreReviews']:
        persistent['i'] += 1
        binding = functools.partial(moreReviewHandler, persistent,
                                    urls['name'])
        http_client.fetch(urls['url'], binding, method="POST",
                          body=urlencode(urls['data']))

    ioloop.IOLoop.instance().start()
    print persistent['results']
    print reviews_db
    reviews_db.insert({'keyword': keyword, 'place': place,
                       'results': persistent['results']}, check_keys=False)
...
def main(keyword, place, entityType='HOTEL'):
    ...
    if reviews_db.count({'keyword': keyword, 'place': place}) > 0:
        ...
        return reviews_db.find_one({'keyword': keyword.lower(),
                                    'place': place.lower()})
